    ...
    public sealed class NullabilityInfoContext
    {
        ...
        private static MemberInfo GetMemberMetadataDefinition(MemberInfo member)
        {
            ...
            if ((type != null) && type.IsGenericType && !type.IsGenericTypeDefinition)
            {
                ...
                var members = type.GetGenericTypeDefinition().GetMembers(all);

                foreach (MemberInfo myMemberInfo in members)
                {
                ...
                throw new ArgumentException(string.Format("A MemberInfo that matches '{0}' could not be found.", member.Name), nameof(member));
            }
